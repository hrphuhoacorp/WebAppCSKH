...
{
    Task<DashboardDTO> GetDashboardForOnlineAsync(DashboardFilter filter);
    Task<DashboardDTO> GetDashboardForSalesAsync(DashboardFilter filter);
}
...
public class DashboardService : IDashboardService
{
    ...
    public async Task<DashboardDTO> GetDashboardForOnlineAsync(DashboardFilter filter)
    {
        try
        {
    ...
    public async Task<DashboardDTO> GetDashboardForSalesAsync(DashboardFilter filter)
    {
        try
        {
            var ordersQuery = _orderRepository
            ...
            var revenueByCategory = await ordersQuery
                ...
                .GroupBy(i => i.Category == null || i.Category == "" ? "Không phân loại" : i.Category)
                .Select(g => new ChartItemDTO
                {
                    Name = g.Key,
                    Value = Math.Round(g.Sum(i => i.Revenue), 0),
                })
                .Where(x => x.Value > 0)
                .OrderByDescending(x => x.Value)
                .ToListAsync();
