...
public class VppStockCountController : ControllerBase
{
        ...
        [FromQuery] int? month, [FromQuery] int? year)
    {
        ...
        return new ResponseValue<IEnumerable<VppStockCountDto>>(result, "OK", StatusReponse.Success);
    }
    ...
    public async Task<ResponseValue<VppStockCountDetailDto>> GetById(int id)
    {
        ...
        if (result == null)
            return new ResponseValue<VppStockCountDetailDto>(null, "Không tìm thấy phiếu kiểm kho", StatusReponse.Error);
        return new ResponseValue<VppStockCountDetailDto>(result, "OK", StatusReponse.Success);
    }

    [HttpPost]
    public async Task<ResponseValue<VppStockCountDetailDto>> Create([FromBody] VppStockCountCreateDto dto)
    {
        var name = User.FindFirstValue(ClaimTypes.Name) ?? "";
        var result = await _service.CreateAsync(dto, name);
        return new ResponseValue<VppStockCountDetailDto>(result, "Tạo phiếu kiểm kho thành công", StatusReponse.Success);
    }

    [HttpPut("{id:int}/lines/{lineId:int}")]
    public async Task<ResponseValue<object>> UpdateLine(
        int id, int lineId, [FromBody] VppStockCountLineUpdateDto dto)
    {
        await _service.UpdateLineAsync(id, lineId, dto.ActualQty, dto.Note);
        return new ResponseValue<object>(new { success = true }, "Cập nhật thành công", StatusReponse.Success);
    }
    ...
    public async Task<ResponseValue<VppStockCountDetailDto>> Confirm(int id)
    {
        ...
        return new ResponseValue<VppStockCountDetailDto>(result, "Đã xác nhận phiếu kiểm kho", StatusReponse.Success);
    }
}
