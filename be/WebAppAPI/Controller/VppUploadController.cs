...
public class VppUploadController : ControllerBase
{
    ...
    [HttpPost]
    public async Task<ResponseValue<List<VppFileUploadResultDto>>> Upload(
        [FromForm] List<IFormFile> files
    )
    {
        var rootPath =
            _config["MediaSettings:RootPath"]
        ...
        foreach (var file in files)
        {
            ...
            if (!_allowed.Contains(ext))
                throw new BadRequestException(
                    $"Loại file không hỗ trợ: {ext}. Chấp nhận: PDF, Excel, Word, ảnh"
                );
        ...
        return new ResponseValue<List<VppFileUploadResultDto>>(
            result,
            "Tải lên thành công",
