...
import { LocalOffer, Close, Person, Phone, CalendarToday, ShoppingBag, AccessTime, KeyboardArrowDown, ArrowUpward, ArrowDownward, UnfoldMore } from '@mui/icons-material';
...
export default function CampaignPerformanceDialog({ open, tagId, onClose }: {
    ...
    const [careSortDays, setCareSortDays] = useState<'asc' | 'desc' | null>(null);
    ...
    const { data: careResult, isLoading: loadingCare } = useQuery({
        queryKey: ['fb-campaign-care-opportunities', tagId, careTagId, careDaysFilter?.label, careSortDays, carePage, carePageSize],
        queryFn: () => fbCampaignTagApi.getCareOpportunities(tagId!, { personaTagId: careTagId ?? undefined, minDays: careDaysFilter?.minDays, maxDays: careDaysFilter?.maxDays, sortByDays: careSortDays ?? undefined, page: carePage + 1, pageSize: carePageSize }),
        enabled: open && !!tagId,
    });
    ...
    return (
        <>
            <Dialog open={open} onClose={handleClose} maxWidth="xl" fullWidth slotProps={{ paper: { sx: { borderRadius: CARD_RADIUS, maxHeight: '92vh' } } }}>
                <DialogTitle sx={{ pb: 1 }}>
                    <Box>
                        <Typography sx={{ fontSize: 30, fontWeight: 800, color: '#1e293b' }}>Hiệu suất chiến dịch</Typography>
                        {perf && (
                                                ...
                                                <TableHead>
                                                    <TableRow sx={{ bgcolor: '#f8fafc' }}>
                                                        ...
                                                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5, cursor: 'pointer', userSelect: 'none' }}
                                                            onClick={() => { setCareSortDays(s => s === 'desc' ? 'asc' : 'desc'); setCarePage(0); }}>
                                                            Lần cuối mua
                                                            {careSortDays === 'asc' ? <ArrowUpward sx={{ fontSize: 14, color: FB }} /> : careSortDays === 'desc' ? <ArrowDownward sx={{ fontSize: 14, color: FB }} /> : <UnfoldMore sx={{ fontSize: 14, color: '#94a3b8' }} />}
                                                        </Box>
                                                    </TableCell>
                                                    </TableRow>
                                                </TableHead>
                                                    ...
                                                    {loadingCare ? (
                                                        ...
                                                        return (
                                                            <TableRow key={c.id} hover sx={dormant ? { bgcolor: alpha(RED, 0.03) } : undefined}>
                                                                <TableCell sx={{ fontSize: 12.5, fontWeight: 700, color: FB, cursor: 'pointer', '&:hover': { textDecoration: 'underline' } }}
                                                                    onClick={() => setDrawerCustomer({ id: c.id, customerCode: c.customerCode, name: c.name, phone: c.phone, totalRevenue: c.totalRevenue, signature: c.signature, tags: c.tags?.map(t => ({ tagName: t.tagName, tagColor: t.tagColor })), categoryAffinityRevenue: c.categoryAffinityRevenue, daysSinceLastOrder: c.daysSinceLastOrder })}>
                                                                    {c.customerCode}
                                                                </TableCell>
                                                                <TableCell sx={{ fontSize: 12.5 }}>{c.name}</TableCell>
