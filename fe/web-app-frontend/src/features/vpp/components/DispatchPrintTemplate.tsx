...
import type { VppDispatchDetailDto } from '../api/vpp.api';

interface Props {
    data: VppDispatchDetailDto | undefined;
}
...
export const DispatchPrintView = React.forwardRef<HTMLDivElement, Props>(({ data }, ref) => {
    if (!data) return null;
    ...
    return (
        ...
        >
            ...
            <Box sx={{ textAlign: 'center', mb: 3 }}>
                <Typography variant="h6" sx={{ fontWeight: 'bold', textTransform: 'uppercase' }}>Phiếu xuất kho</Typography>
                <Typography variant="body2">Số: {data.code} - Ngày xuất: {fmtDate(data.dispatchDate)}</Typography>
                <Typography variant="body2">
                    {data.department && `Bộ phận nhận: ${data.department}`}
                    {data.department && data.branch && ' — '}
                    {data.branch && `Chi nhánh: ${data.branch}`}
                </Typography>
...
DispatchPrintView.displayName = 'DispatchPrintView';
