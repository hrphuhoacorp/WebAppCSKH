...
import { vppApi, VppImportCreateDto, VPP_GREEN } from '../api/vpp.api';
...
interface ImportLine { itemId: number; quantity: number; unitPrice: number; }
...
export default function TabImport() {
    ...
    const [lines, setLines] = useState<ImportLine[]>([{ itemId: 0, quantity: 1, unitPrice: 0 }]);
    ...
    function resetForm() { setNote(''); setImportDate(now.toISOString().split('T')[0]); setLines([{ itemId: 0, quantity: 1, unitPrice: 0 }]); }
    function updateLine(idx: number, patch: Partial<ImportLine>) { setLines(prev => prev.map((l, i) => i === idx ? { ...l, ...patch } : l)); }
    function handleCreate() {
        const valid = lines.filter(l => l.itemId > 0 && l.quantity > 0);
        if (!valid.length) { toast.error('Cần ít nhất 1 dòng hợp lệ'); return; }
        createMut.mutate({ importDate, note: note || undefined, lines: valid });
    }
                    ...
                    <TableBody>
                        {isLoading ? (
                            ...
                            <TableRow><TableCell colSpan={8} align="center" sx={{ py: 8 }}>
                                <Typography sx={{ color: '#94a3b8', fontSize: 14 }}>Chưa có phiếu nhập trong tháng {month}/{year}</Typography>
                            </TableCell></TableRow>
                        ) : imports.map((imp, i) => (
