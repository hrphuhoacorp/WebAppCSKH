...
function HeroArticle({ item, onClick }: { item: NewsItem; onClick: () => void }) {
    if (item.thumbnailUrl) {
        return (
            <Box onClick={onClick} sx={{ position: 'relative', overflow: 'hidden', cursor: 'pointer', '&:hover .hero-img': { transform: 'scale(1.025)' }, '&:hover .hero-title': { color: '#86efac' } }}>
                <Box sx={{ position: 'relative', width: '100%', height: { xs: 280, sm: 420, md: 560 }, overflow: 'hidden' }}>
                    <Image className="hero-img" src={item.thumbnailUrl} alt={item.title} fill priority sizes="100vw" style={{ objectFit: 'cover', transition: 'transform 0.8s cubic-bezier(0.4,0,0.2,1)' }} />
                </Box>
                ...
                <Box sx={{ position: 'absolute', bottom: 0, left: 0, right: 0, px: { xs: 2.5, md: 6, lg: 10 }, pb: { xs: 4, md: 6 } }}>
                    ...
                    <Typography className="hero-title" sx={{ fontWeight: 900, fontSize: { xs: 22, sm: 30, md: 42, lg: 50 }, letterSpacing: '-0.03em', lineHeight: 1.1, color: '#fff', mt: 1.5, mb: 2, transition: 'color 0.25s', textShadow: '0 2px 16px rgba(0,0,0,0.4)', display: '-webkit-box', WebkitLineClamp: 3, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
                        {item.title}
                    </Typography>
    ...
    return (
        <Box onClick={onClick} sx={{ px: { xs: 2.5, md: 6, lg: 10 }, py: { xs: 5, md: 8 }, cursor: 'pointer', borderBottom: '1px solid rgba(13,43,30,0.08)', borderLeft: typeInfo ? `4px solid ${typeInfo.color}` : '4px solid #086839', pl: { xs: 3, md: 7, lg: 11 }, '&:hover .hero-title-text': { color: '#086839' } }}>
            <TypeBadge type={item.type} pinned={item.isPinned} />
            <Typography className="hero-title-text" sx={{ fontWeight: 900, fontSize: { xs: 28, md: 48 }, letterSpacing: '-0.03em', lineHeight: 1.1, color: '#0f172a', mt: 2, mb: 2.5, transition: 'color 0.2s' }}>
                {item.title}
            </Typography>
...
function ArticleCard({ item, onClick }: { item: NewsItem; onClick: () => void }) {
    return (
        <Box onClick={onClick} sx={{ cursor: 'pointer', '&:hover .card-title': { color: '#086839' }, '&:hover .card-img': { transform: 'scale(1.04)' } }}>
            <Box sx={{ overflow: 'hidden', borderRadius: '6px', mb: 2, bgcolor: '#f1f5f9' }}>
                {item.thumbnailUrl ? (
                    <Box sx={{ position: 'relative', width: '100%', aspectRatio: '16/9' }}>
                        <Image className="card-img" src={item.thumbnailUrl} alt={item.title} fill sizes="(max-width: 600px) 100vw, (max-width: 900px) 50vw, 33vw" style={{ objectFit: 'cover', transition: 'transform 0.55s cubic-bezier(0.4,0,0.2,1)' }} />
                    </Box>
                ) : (
                    <Box sx={{ width: '100%', aspectRatio: '16/9', display: 'flex', alignItems: 'center', justifyContent: 'center', background: item.type && TYPE_LABEL[item.type] ? `linear-gradient(135deg, ${TYPE_LABEL[item.type].color}22, ${TYPE_LABEL[item.type].color}44)` : 'linear-gradient(135deg, #08683922, #08683944)' }}>
                        <Typography sx={{ fontSize: 32, opacity: 0.5 }}>📰</Typography>
                    </Box>
                )}
            </Box>
            <TypeBadge type={item.type} pinned={item.isPinned} />
            <Typography className="card-title" sx={{ fontWeight: 800, fontSize: { xs: 16, md: 18 }, letterSpacing: '-0.015em', lineHeight: 1.35, color: '#0f172a', mt: 1.2, mb: 1.2, transition: 'color 0.2s', display: '-webkit-box', WebkitLineClamp: 3, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
                {item.title}
            </Typography>
...
function FeedItem({ item, onClick }: { item: NewsItem; onClick: () => void }) {
    return (
        <Box onClick={onClick} sx={{ py: { xs: 3, md: 3.5 }, borderBottom: '1px solid rgba(13,43,30,0.07)', display: 'grid', gridTemplateColumns: item.thumbnailUrl ? { xs: '1fr 90px', md: '1fr 160px' } : '1fr', gap: { xs: 2, md: 4 }, alignItems: 'center', cursor: 'pointer', '&:hover .feed-title': { color: '#086839' }, '&:hover .feed-img': { transform: 'scale(1.04)' } }}>
            <Box sx={{ minWidth: 0 }}>
                <TypeBadge type={item.type} pinned={item.isPinned} />
                <Typography className="feed-title" sx={{ fontWeight: 800, fontSize: { xs: 16, md: 20 }, letterSpacing: '-0.015em', lineHeight: 1.3, color: '#0f172a', mt: 1, mb: 1, transition: 'color 0.2s', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
                    {item.title}
                </Typography>
            ...
            {item.thumbnailUrl && (
                <Box sx={{ position: 'relative', overflow: 'hidden', borderRadius: '6px', flexShrink: 0, height: { xs: 72, md: 110 } }}>
                    <Image className="feed-img" src={item.thumbnailUrl} alt={item.title} fill sizes="(max-width: 768px) 90px, 160px" style={{ objectFit: 'cover', transition: 'transform 0.5s cubic-bezier(0.4,0,0.2,1)' }} />
                </Box>
            )}
        </Box>
    );
...
function SectionTitle({ title }: { title: string }) {
    return (
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 0 }}>
            ...
            <Typography sx={{ fontWeight: 900, fontSize: { xs: 17, md: 20 }, letterSpacing: '-0.02em', color: '#0f172a' }}>{title}</Typography>
...
export default function HomeNewsClient({ initialItems, initialTotal, initialCategory, initialSearch, initialPage }: Props) {
    ...
    const isFiltering = initialCategory !== 'all' || initialSearch !== '';
    const today = new Date().toLocaleDateString('vi-VN', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' });

    const hero = initialItems[0];
    const gridItems = initialItems.slice(1, 4);
    const feedItems = initialItems.slice(4);

    const buildUrl = (overrides: { search?: string; category?: string; page?: number }) => {
        const s = overrides.search ?? initialSearch;
        const c = overrides.category ?? initialCategory;
    ...
    const handleSearchChange = (value: string) => {
        ...
        debounceRef.current = setTimeout(() => {
            startTransition(() => router.push(buildUrl({ search: value.trim(), page: 1 })));
        }, 350);
    };
    ...
    return (
        <>
            ...
            {/* ── Masthead strip ── */}
            <Box sx={{ px: { xs: 2.5, md: 5, lg: 8 }, height: { xs: 48, md: 52 }, display: 'flex', alignItems: 'center', gap: 2, borderBottom: '1px solid rgba(13,43,30,0.08)' }}>
                ...
                {isFiltering && (
                    <Box sx={{ display: { xs: 'none', md: 'flex' }, alignItems: 'center', gap: 1.5 }}>
                        {initialCategory !== 'all' && TYPE_LABEL[initialCategory] && (
                            <Box sx={{ px: 1.4, py: 0.35, borderRadius: '5px', bgcolor: TYPE_LABEL[initialCategory].color + '14', color: TYPE_LABEL[initialCategory].color, fontSize: 11.5, fontWeight: 800, letterSpacing: 1.5, textTransform: 'uppercase' }}>
                                {TYPE_LABEL[initialCategory].label}
                            </Box>
                        )}
                        {initialSearch && (
                            <Typography sx={{ fontSize: 12.5, color: '#64748b', fontStyle: 'italic' }}>
                                Kết quả cho &ldquo;{initialSearch}&rdquo;
                            </Typography>
                        )}
                    </Box>
                )}
                ...
                {/* Search */}
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.8, px: 1.8, height: 36, borderRadius: '8px', border: '1px solid rgba(13,43,30,0.1)', width: { xs: 180, sm: 240, md: 280 }, maxWidth: 320, bgcolor: '#f8fafc', transition: 'border-color 0.2s, box-shadow 0.2s, background-color 0.2s', '&:focus-within': { borderColor: '#086839', boxShadow: '0 0 0 3px rgba(8,104,57,0.08)', bgcolor: '#fff' } }}>
                    <SearchRoundedIcon sx={{ fontSize: 15, color: '#94a3b8', flexShrink: 0 }} />
                    <InputBase placeholder="Tìm kiếm bài viết..." value={search} onChange={e => handleSearchChange(e.target.value)} sx={{ fontSize: 13, flex: 1, color: '#0f172a' }} />
                    {search && <IconButton size="small" onClick={() => handleSearchChange('')} sx={{ p: 0.25 }}><CloseRoundedIcon sx={{ fontSize: 13, color: '#94a3b8' }} /></IconButton>}
                </Box>
            </Box>

            {/* Filter-change loading pill */}
            {isPending && (
            ...
            {/* ── Filter results ── */}
            {initialItems.length > 0 && isFiltering && (
                <Box sx={{ px: { xs: 2.5, md: 6, lg: 10 }, pt: 3, pb: { xs: 6, md: 10 } }}>
                    <Typography sx={{ fontSize: 13, color: '#94a3b8', fontWeight: 600, mb: 3 }}>
                        Tìm thấy <Box component="b" sx={{ color: '#086839' }}>{initialTotal}</Box> bài viết
                    </Typography>
                    <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: '1fr 1fr', md: '1fr 1fr 1fr' }, gap: { xs: 4, md: 5 } }}>
                        {initialItems.map(item => <ArticleCard key={item.id} item={item} onClick={() => go(item)} />)}
                    </Box>
                    {totalPages > 1 && (
                        <Box sx={{ display: 'flex', justifyContent: 'center', mt: 6 }}>
                            <Pagination count={totalPages} page={initialPage} onChange={handlePageChange} shape="rounded" sx={{ '& .Mui-selected': { bgcolor: '#086839 !important', color: '#fff' } }} />
                        </Box>
                    )}
                </Box>
            )}
